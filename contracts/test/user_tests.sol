...
contract TestUser{
    ...
    function beforeAll() public {
        ...
        (id0, acc0, username0, active0, canVote0) = user.retrieve(0);
    }
    ...
    function testCreateOwnerIsOwnerOfContract() public {
        ...
        Assert.equal("vanities", username0, "get owner returns the username of the owner");
...
user.create("some-username");
        (id1, acc1, username1, active1, canVote1) =  user.retrieve(1);
        Assert.equal(id1, uint(1), "get owner returns the id of the user");
        Assert.equal(acc1, acc1, "get owner returns the address of the user");
        Assert.equal(username1, string("some-username"), "get owner returns the username of the user");


user.create("some-username");
        (id2, acc2, username2, active2, canVote2) = user.retrieve(2);
        Assert.equal(id1, uint(1), "get owner returns the id of the user");
        Assert.equal(acc2, acc2, "get owner returns the address of the user");
        Assert.equal(username2, string("some-username"), "get owner returns the username of the user");
    }
}
