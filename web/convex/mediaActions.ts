...
export const upload = action({
  args: { bytes: v.bytes(), mime: v.string() },
  returns: v.object({ hash: v.string(), mime: v.string(), size: v.number() }),
  handler: async (ctx, { bytes, mime }): Promise<{ hash: string; mime: string; size: number }> => {
    const size = bytes.byteLength;
    if (size === 0) throw new Error("empty file");
    if (size > MAX_BYTES) throw new Error(`file too large (max ${MAX_BYTES / 1024 / 1024} MB)`);
    if (mime.startsWith("video/")) throw new Error("video isn't supported yet");
    ...
    console.log(`[media] stored ${hash} (${mime}, ${size}b)`);
